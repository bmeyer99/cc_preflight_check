...
RESOURCE_ACTION_MAP = {
    "AWS::IAM::Role": {
        ...
        "property_actions": {
            ...
            "AssumeRolePolicyDocument": [] # CreateRole handles this
        },
        "pass_role_to_services": [] # e.g., if role is passed to EC2, Lambda
    },
    "AWS::IAM::Policy": { # For standalone policies, not inline
    ...
    "AWS::Lambda::Function": {
        "generic_actions": [
            ...
            "lambda:TagResource",
            "lambda:UpdateFunctionConfiguration", # Often needed for setting env vars, etc.
            "lambda:UpdateFunctionCode"
        ],
        "arn_pattern": "arn:aws:lambda:{region}:{accountId}:function:{functionName}",
        "property_actions": {
            "Role": ["iam:PassRole"] # Permission to pass the specified role to Lambda
        },
    ...
    "AWS::Lambda::Permission": {
        ...
        "arn_pattern": "arn:aws:lambda:{region}:{accountId}:function:{functionName}", # Applies to the function
        "property_actions": {}
    },
    "AWS::S3::Bucket": {
        "generic_actions": [
            ...
            "s3:DeleteBucket",
            "s3:PutBucketTagging",
            "s3:PutEncryptionConfiguration", # For BucketEncryption
            "s3:PutLifecycleConfiguration", # For LifecycleConfiguration
            "s3:PutBucketPolicy", # For BucketPolicy (though BucketPolicy is a separate resource type too)
            "s3:PutBucketVersioning",
            "s3:PutAccelerateConfiguration",
            "s3:PutBucketAcl", # Generally avoid ACLs if possible
            "s3:GetBucketAcl"  # Often needed by CFN
        ],
    ...
    "AWS::S3::BucketPolicy": {
        ...
        "arn_pattern": "arn:aws:s3:::{bucketName}", # Applies to the bucket
        "property_actions": {}
    },
    "AWS::SQS::Queue": {
        "generic_actions": ["sqs:CreateQueue", "sqs:DeleteQueue", "sqs:TagQueue", "sqs:SetQueueAttributes"],
        "arn_pattern": "arn:aws:sqs:{region}:{accountId}:{queueName}",
        "property_actions": {}
    },
    "AWS::SQS::QueuePolicy": {
        "generic_actions": ["sqs:SetQueueAttributes"], # Policy is an attribute
        "arn_pattern": "arn:aws:sqs:{region}:{accountId}:{queueName}", # Applies to the queue
        "property_actions": {}
    },
    "AWS::SNS::Topic": {
        "generic_actions": ["sns:CreateTopic", "sns:DeleteTopic", "sns:TagResource", "sns:SetTopicAttributes"],
        "arn_pattern": "arn:aws:sns:{region}:{accountId}:{topicName}",
        "property_actions": {}
    },
    "AWS::SNS::TopicPolicy": {
        "generic_actions": ["sns:SetTopicAttributes"], # Policy is an attribute
        "arn_pattern": "arn:aws:sns:{region}:{accountId}:{topicName}", # Applies to the topic
        "property_actions": {}
    },
    "AWS::SNS::Subscription": {
        "generic_actions": ["sns:Subscribe", "sns:Unsubscribe"], # Subscribe also needs permission from endpoint (e.g. SQS:SendMessage)
        "arn_pattern": "arn:aws:sns:{region}:{accountId}:{topicName}:*", # Subscription ARN is dynamic
        "property_actions": {
            "Endpoint": [], # If SQS, SQS queue policy needs to allow SNS
            "Protocol": []
        }
    },
    "AWS::KMS::Key": {
        "generic_actions": [
            "kms:CreateKey",
            "kms:TagResource",
            "kms:ScheduleKeyDeletion", # For deletion
            "kms:PutKeyPolicy" # Policy is often set during creation
        ],
        "arn_pattern": "arn:aws:kms:{region}:{accountId}:key/*", # Key ID is generated
        "property_actions": {}
    },
    "AWS::KMS::Alias": {
    ...
    "AWS::CloudTrail::Trail": {
        ...
        "property_actions": {
            "S3BucketName": ["s3:PutObject", "s3:GetBucketPolicy", "s3:GetBucketAcl"], # Trail needs to write to S3
            "KMSKeyId": ["kms:DescribeKey", "kms:GenerateDataKey*", "kms:Decrypt"], # Permissions for CMK
            "CloudWatchLogsLogGroupArn": ["logs:CreateLogStream", "logs:PutLogEvents", "logs:DescribeLogGroups"], # If logging to CWL
            "RoleArn": ["iam:PassRole"] # If a custom role is used for CWL
        }
    },
    "AWS::CloudFormation::StackSet": {
        "generic_actions": [
            ...
            "iam:PassRole" # For AdministrationRoleARN and ExecutionRoleName
        ],
    ...
    "Custom::PublishRoleDetail": { "type": "CustomResource" },
...
def resolve_value(value, parameters, account_id, region, resources_in_template):
    if isinstance(value, dict):
        if "Ref" in value:
            ref_val = value["Ref"]
            if ref_val == "AWS::AccountId":
                return account_id
            if ref_val == "AWS::Region":
                return region
            if ref_val in parameters:
                return parameters[ref_val]
            # Basic Ref to another resource in the template (return the logical ID for now)
            if ref_val in resources_in_template:
                return ref_val # Or a placeholder ARN pattern if possible
            print(f"Warning: Unresolved Ref: {ref_val}", file=sys.stderr)
            return str(value) # Or some placeholder
